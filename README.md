...
- 独立工作台：模板、历史、快捷键、输出、外观、数据集中管理
- 后台托盘运行：可从托盘打开编辑窗口、工作台或退出应用
- 单实例运行：重复打开 exe 时会唤起工作台
...
编辑窗口保持低干扰：主体是文本编辑区，底部显示保存状态、字符数、估算 token、历史数量和快捷键提示。右上角齿轮按钮会打开独立工作台。
...
工作台用于低频配置和内容维护。关闭工作台时会隐藏到后台，再次通过齿轮按钮、托盘菜单或重复打开 exe 都可以重新显示。
