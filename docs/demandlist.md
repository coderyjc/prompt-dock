...
- 用户可在工作台中选择“总是恢复最后内容”或“提交后保留当前内容”。

数据保留策略：

- 当前草稿永久保留，直到用户在工作台中删除或被新草稿覆盖。
- 完成历史默认保留 30 天，可关闭。
- 支持一键清除历史与缓存。
...
- 居中或出现在当前输入区域附近。
- 窗口置顶，失焦后按工作台中的设置自动隐藏。
- 默认隐藏标题栏和菜单栏，视觉上只保留编辑区。
- 支持所有核心快捷键：提交、复制、模板选择、历史草稿、隐藏。
- 角落保留一个进入工作台的图标按钮，使用齿轮或侧栏图标，悬停显示名称。
...
- 默认宽 920px，高 640px。
- 分两栏显示：左栏导航栏，右栏内容栏。
- 工作台用于模板、历史、快捷键、输出、外观、数据管理等低频操作。
...
工作台就是设置与内容管理页，通过编辑窗口角落图标、托盘菜单或 `Ctrl+,` 打开。
...
| 工作台打开 | damping `1.0`, response `0.3-0.4` |
...
| 剪贴板覆盖用户内容 | 提交后延迟恢复原剪贴板，可在工作台中关闭 |
